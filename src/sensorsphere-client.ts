...
import type { ManagedAgentManager, ManagedAssignment, ManagedOperationProgress } from "./manager.js";
...
export class SensorSphereSupervisorClient {
  ...
  private async handleMessage(text: string): Promise<void> {
    ...
    const progress = (event: ManagedOperationProgress): void => {
      if (this.socket?.readyState !== WebSocket.OPEN) return;
      this.socket.send(JSON.stringify({
        type: "SUPERVISOR_COMMAND_PROGRESS",
    ...
    try {
      switch (message.operation) {
        ...
        case "DEPLOY":
          ...
          result = await this.manager.deploy(message.agentType, message.instance ?? "main", message.version, message.environment ?? {}, { commandId: message.commandId, onProgress: progress });
          break;
        case "UPDATE":
          if (!message.agentType || !message.version) throw new Error("agentType and version are required");
          ...
          result = await this.manager.update(message.version, message.agentType, message.instance ?? "main", { commandId: message.commandId, onProgress: progress });
          break;
        case "REMOVE":
          if (!message.agentType) throw new Error("agentType is required");
          result = await this.manager.remove(message.agentType, message.instance ?? "main", { commandId: message.commandId, onProgress: progress });
          break;
